...
- **Jeu de groupe :**  \nLe mode standard de jeu social. Great for friends, families, or small groups - up to 100 players. Inclus avec un compte Premium. Aucun seat n'est requis. Le jeu s'exécute automatiquement : l'hôte peut le mettre en pause, mais n'a pas d'autre contrôle sur le rythme.
...
QuizWitz is designed as a Party Game to be played with friends, family, or colleagues - either in the same room or via a video call. Il n'est pas possible de rejoindre une partie lancée par quelqu'un que vous ne connaissez pas.
...
In Party and Conference games, pausing is the only way to interrupt the game - everything else runs automatically. Si vous voulez plus de contrôle, utilisez le mode de jeu **QuizWitz Live** avec la Quizmaster App. Cela vous donne un contrôle total sur le rythme et le déroulement de votre quiz.
