...
The game has dozens of distinct screen states, but most are variants of the same layout. **You design seven frames; the rest is derived from them.** That is not a shortcut - it is how the engine works. A screen with no artwork of its own falls back to a general frame.
...
| **2** | **Connect screen** | What the room sees to join: five lines of instruction, a join code, a QR code, a line with the number of connected players, and a list of players trickling in. Design it **twice**: with a client logo and without one. The join code and the QR code are generated by the engine; reserve a square for the QR code. | Also the waiting screen shown while the quizmaster reads the question aloud, again with and without a logo. |
...
The screens below come from an existing theme. They are here to show **what happens on each screen**; they are not a style reference. All of them, and more, can be played in the [theme tester](https://client.quizwitz.com/test.html).
...
3. **Each separate graphic element as a transparent PNG at 2×**, in one folder, filename matching the layer name.
4. **The timer** as keyframes or a written description of the progression.
5. **Fonts** as `.ttf` or `.otf`, with proof of licence.
6. **The colour list** described above, as hex values.
7. **Half a page of notes**: what the idea is, how the options should appear, what moves and what stays still. Not a ten-page design rationale - whoever builds the theme needs to know what to build. Motion ideas may be described or supplied as a rough animatic.

### Order of work

Start with frame 3, the question screen, and get it approved before the rest. It carries the timer, the options and a button, so it settles the style of nearly every control, and it is the screen the room looks at longest. Frames 1 and 2 follow naturally from it; frames 5 to 7 come last.
...
|---|---|---|
| 1. General frame | `GeneralPurposeScreen`; `GeneralPurposeScreenWithHeader` optional | `header.text`, content area |
